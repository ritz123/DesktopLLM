...
export default function App() {
  ...
  useEffect(() => {
    window.desktopLLM.getSettings().then((settings) => {
      ...
      setHasOpenRouterKey(settings.hasOpenRouterKey);
  ...
  async function saveSettings() {
    ...
    if (openRouterKey) setHasOpenRouterKey(true);
  ...
  return <main className={`app-shell theme-${theme} ${sidebarHidden ? "sidebar-hidden" : ""} ${inspectorOpen && activeView === "chat" ? "inspector-open" : ""} ${activeView === "coding" ? "coding-view" : ""}`} style={shellStyle}>
    ...
    {aboutOpen && <div className="modal-backdrop"><section className="about-dialog" role="dialog" aria-modal="true" aria-labelledby="about-title"><img src="../build/icon.png" alt="" /><p className="eyebrow">DesktopLLM</p><h2 id="about-title">Local-first AI workspace</h2><p>Version {__APP_VERSION__}</p><p>Chat with Ollama models on this device or OpenRouter models online. Conversations stay local; OpenRouter keys use OS encryption.</p><p>Select a work folder in chat so compatible Ollama models can read, write, and run commands there.</p><p className="license">Licensed under GNU GPL v3.0</p><p className="copyright">© 2026 Biplab Sarkar</p><footer><button className="primary" onClick={() => setAboutOpen(false)}>Close</button></footer></section></div>}
  </main>;
}
